...
router.get('/', async function (req, res, next) {
    ...
    filterData.select = {
        ['display']: {
            count: await _model.count('display', false, {}, {params: params, task: 'filter'}),
            data: _form.createSelectData(['display'], [true], ['all display']).display,
        },
    }
    res.render(viewFolder+'/index', {
        pageTitle: "List Page",
        items: await _model.listItems(params, {task: 'list-all'}),
        filterData, params, require, controller,
    });  
});
...
// CHANGE IS HOME
router.get('/is_home/:type(\\w+)/:id([a-z0-9]+)', async (req, res, next) => {
    let result  = await _model.saveItem({type: req.params.type, id: req.params.id, user: req.user}, {task: 'change-is_home'});
    res.json(result);
});

// CHANGE DISPLAY
router.get('/display/:type(\\w+)/:id([a-z0-9]+)', async (req, res, next) => {
    let result  = await _model.saveItem({type: req.params.type, id: req.params.id, user: req.user}, {task: 'change-display'});
    res.json(result);
});
...
router.post('/save/:id(\\w+)?', _validate, async (req, res) => {
    ...
    // validate success
        ...
        let item    = {
            name: req.body.name,
            slug: _helper.slug(req.body.slug),
...
router.post('/savemulti/:type(delete|status)/:value(\\w+)?', async (req, res, next) => {
    ...
    if (req.body){
        if (typeof req.body.id == 'string') req.body.id = [req.body.id];
        let rowsAffected;
        if (type == 'status'){
