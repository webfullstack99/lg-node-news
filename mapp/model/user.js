...
let currentModel = {
    ...
    getItem: function (params, options) {
        let result;
        switch (options.task) {
            case 'get-by-google-id':
                result = this.getItemByGoogleId(params, options);
                break;
    ...
    getItemByGoogleId: async function (params, options) {
        let result = await this._model.findOne({ googleId: params.googleId });
        return result;
    },

    findOrCreateItemGoogleLogin: async function (profile) {
        let item = await this.getItem({ googleId: profile.id }, { task: 'get-by-google-id' });
        if (!item) {
            item = this.createItemByProfile(profile);
            let saveResult = await new this._model(item).save();
            console.log('\n created\n');
        }else{
            console.log('\nfind\n');
    ...
    createItemByProfile: function (profile) {
        let item = {
            googleId: profile.id,
